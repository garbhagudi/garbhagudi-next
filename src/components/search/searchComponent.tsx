...
export default function SearchComponent() {
  ...
  return (
    <div className='w-80 bg-gray-100 dark:bg-gray-800 outline-none mx-auto mt-10'>
      <Combobox value={selected} onChange={setSelected}>
        <div className='relative mt-1'>
          <div className='relative w-full cursor-default overflow-hidden  text-left shadow-lg focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300 sm:text-sm'>
            <Combobox.Input
              className='w-full py-3 px-3 font-content pr-10 text-lg rounded-xl bg-gray-50 dark:bg-gray-700 leading-5 text-gray-800 dark:text-gray-200 border-2 border-brandPink focus:outline-none focus:ring-0'
          ...
          >
            <Combobox.Options className='absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white dark:bg-gray-800 py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm'>
              {searchTerms.length === 0 && query !== '' ? (
                <div className='relative cursor-default select-none py-2 px-4 text-gray-800 dark:bg-gray-200 '>
                  Nothing found.
                </div>
              ) : (
                searchTerms.map((item) => (
                  <Combobox.Option
                    key={item.title}
                    className={({ active }) =>
                      `relative cursor-default select-none py-2 pl-10 font-lexend pr-4 ${
                        active
                          ? 'bg-brandPink dark:bg-gray-600 text-white'
