...
const IndexPage = ({ causes }) => {
  const pageUrl = '/resources/causes';
  const listed = (causes || [])
    .filter((item) => item?.title && item?.slug)
    .map((item) => ({ name: item.title, url: `/resources/causes/${item.slug}` }));

  const schema = schemaGraph(
    ...buildDirectoryPage(pageUrl, 'Causes', listed),
    buildBreadcrumb(pageUrl, [{ text: 'Resources', link: '/resources' }, { text: 'Causes' }])
  );
  return (
    <div>
      <Head>
