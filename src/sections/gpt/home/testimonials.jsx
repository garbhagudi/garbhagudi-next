...
/* Dash's own swiper config had no `autoplay` — only `loop` + `pagination` —
 * so this stays manual-navigation-only, unlike the autoplaying nuka-carousel
 * testimonial sections elsewhere in this repo.
 *
 * Dots hidden: nuka-carousel v7's default paging-dots container is
 * absolutely positioned mid-slide (not pinned to the bottom), so it visibly
 * overlaps slide content here — every other nuka-carousel in this repo
...
const Testimonial = () => {
  return (
    <section className='overflow-hidden bg-gradient-to-tr from-brandPink3/70 via-brandPurple/50 to-purple-100 py-8'>
          ...
          <Carousel
            ...
            renderCenterLeftControls={({ previousSlide }) => (
              <button
                ...
                className='ml-4 hidden rounded-full bg-brandPink p-2 text-white md:block'
              >
            ...
            renderCenterRightControls={({ nextSlide }) => (
              <button
                ...
                className='mr-4 hidden rounded-full bg-brandPink p-2 text-white md:block'
              >
                ...
                <div className='mt-8'>
                  ...
                  <div className='flex items-center justify-center gap-3 pt-4'>
                    <div className='mt-3 text-center md:ml-4 md:mt-0 md:flex md:items-center'>
                      <div className='font-dmSans font-semibold text-gray-900'>{item?.name}</div>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className='mx-auto max-w-6xl py-1 pt-4 text-center font-dmSans text-sm underline lg:text-right'>
