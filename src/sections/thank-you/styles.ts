export const thankYouCss = `
  ...
  /* Redundant with Tailwind Preflight here, but kept identical to the
     public/thank-you.html static copy (which has no Preflight) so both
     stay in sync. */
  #ty-root, #ty-root *, #ty-root *::before, #ty-root *::after {
    box-sizing: border-box;
  }
  #ty-root h1, #ty-root h2, #ty-root h3, #ty-root p {
    margin: 0;
  }
  #ty-root svg {
    display: block;
  ...
  #ty-root nav img { height: 44px; width: auto; }
  #ty-root nav a.phone svg { width: 16px; height: 16px; }
  #ty-root nav a.phone {
